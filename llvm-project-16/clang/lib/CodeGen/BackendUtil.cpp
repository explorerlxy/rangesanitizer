...
void EmitAssemblyHelper::CreateTargetMachine(bool MustCreateTM) {
  ...
  std::string Error;
  std::string TT = TheModule->getTargetTriple();
  ...
  // BaseBounds
  // the large code model globals also assume -no-pie
  Triple t(TheModule->getTargetTriple());
  if(LangOpts.Sanitize.has(SanitizerKind::SafeStack) && t.getArch() == Triple::x86_64) {
  ...
  if (!initTargetOptions(Diags, Options, CodeGenOpts, TargetOpts, LangOpts,
                         HSOpts))
    return;
  TM.reset(TheTarget->createTargetMachine(TT, TargetOpts.CPU, FeaturesStr,
